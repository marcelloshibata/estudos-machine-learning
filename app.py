...
st.markdown("# あなたが幸せかどうか診断しましょう！")

redes_opt = ['LinkedIn', 'Twitch', 'YouTube', 'Instagram', 'Amigos',
       'Twitter / X', 'Outra rede social']
redes = st.selectbox("このチャンネルをどのように知りましたか？", options=redes_opt)
cursos_opt = ['0', '2', '1', '3', 'Mais que 3',]
cursos = st.selectbox("受講したコースの数はいくつですか？", options=cursos_opt)
...
with col1:
    video_game = st.radio("ゲームは好きですか？", ["Sim", "Não"])
    futebol = st.radio("サッカーは好きですか？", ["Sim", "Não"])
    tempo = st.selectbox("データ分野での実務経験年数", options=tempos)

with col2:
    livros = st.radio("本は好きです？", ["Sim", "Não"])
    tabuleiro = st.radio("ボードゲームは好きですか？", ["Sim", "Não"])
    uf = st.selectbox("現在居住しているいる州？", options=ufs)
    cadeira = st.selectbox("役職・シニアリティ", options=niveis)

with col3:
    f1 = st.radio("Ｆ１ゲームは好きですか？", ["Sim", "Não"])
    mma = st.radio("総合格闘技（MMA）は好きですか？", ["Sim", "Não"])
    formacao = st.selectbox("専攻分野", options=areas_formacao)

idade = st.number_input("年齢：", 18, 100)
